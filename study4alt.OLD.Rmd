...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=5, fig.width=5, eval=FALSE}
library(ggplot2)
...
library(tsne)
tSNE.norm <- tsne(loaddata[, c(14,17,20,23,29,35)])

loaddata$tsne1 <- tSNE.norm[,1]
loaddata$tsne2 <- tSNE.norm[,2]
ggplot(loaddata, aes(x=tsne1,y=tsne2, col=CoarseLoad))+geom_point()
...
library(diagram)
tm <- round(t(matrix(getpars(f)[7:42],nrow = 6,ncol = 6)),digits = 3)
plotmat(tm, 
        lwd = 1, box.lwd = 2, 
        cex.txt = 0.8, 
        box.size = 0.1, 
...
```{r, cache=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.height=5, fig.width=15}
library(gplots)

loaddata$ExpSession <- as.factor(paste(loaddata$session,loaddata$value.Experimental))
plotmeans(value.Mean~ExpSession,data=loaddata,connect=F) # The metric behaves as expected...
plotmeans(value.Mean.norm~ExpSession,data=loaddata,connect=F) # The metric behaves as expected...
...
# Saturated model
lmMeanNormSat <- lm(value.Mean.norm ~ value.Activity*value.Social*value.Experimental, data=loaddata)
summary(lmMeanNormSat)  # C1 and C2 have coefficients opposite as expected...
anova(lmMeanNormSat)
...
# Saturated model
lmSDNormSat <- lm(value.SD.norm ~ value.Activity*value.Social*value.Experimental, data=loaddata)
summary(lmSDNormSat)  # C1 and C2 have coefficients opposite as expected...
anova(lmSDNormSat)
...
# Saturated model
lmSacNormSat <- lm(value.Sac.norm ~ value.Activity*value.Social*value.Experimental, data=loaddata)
summary(lmSacNormSat)  # C1 and C2 have coefficients as expected
anova(lmSacNormSat)
...
# Saturated model
lmFixNormSat <- lm(value.Fix.norm ~ value.Activity*value.Social*value.Experimental, data=loaddata)
summary(lmFixNormSat)  # C1 and C2 have coefficients as expected
anova(lmFixNormSat)
...
# Saturated model
lmThetaNormSat <- lm(value.Theta.norm ~ value.Activity*value.Social*value.Experimental, data=loaddata)
summary(lmThetaNormSat)  # C1 and C2 have coefficients opposite as expected
anova(lmThetaNormSat)
...
# Saturated model
lmJerkNormSat <- lm(value.Jerk.Mean.norm ~ value.Activity*value.Social*value.Experimental, data=loaddata)
summary(lmJerkNormSat)  # C1 and C2 have coefficients as expected
anova(lmJerkNormSat)
...
# Saturated model
lmETFineSat <- lm(FineLoad ~ value.Activity*value.Social*value.Experimental, data=loaddataET)
summary(lmETFineSat) # C1 and C2 have coefficients  as expected...
anova(lmETFineSat)
...
# Saturated model
lmETTFineSat <- lm(FineLoad ~ value.Activity*value.Social*value.Experimental, data=loaddataETT)
summary(lmETTFineSat) # C1 and C2 have coefficients opposite as expected...
anova(lmETTFineSat)
...
# Saturated model
lmETTAFineSat <- lm(FineLoad ~ value.Activity*value.Social*value.Experimental, data=loaddataETTA)
summary(lmETTAFineSat) # C1 and C2 have coefficients as expected...
anova(lmETTAFineSat)
...
# Saturated model
lmPCAxSat <- lm(Dim1 ~ value.Activity*value.Social*value.Experimental, data=loaddata)
summary(lmPCAxSat) # C1 and C2 have coefficients  as expected...
anova(lmPCAxSat)
...
wald.test(b = coef(lmPCAx), Sigma = vcov(lmPCAx), L = l)  # ... And the difference is  significant
# Saturated model
lmPCAxSat <- lm(Dim1Norm ~ value.Activity*value.Social*value.Experimental, data=loaddata)
summary(lmPCAxSat) # C1 and C2 have coefficients  as expected...
anova(lmPCAxSat)
...
## Conclusions for measuring orchestration load

...

## Classroom usability insights from this study
